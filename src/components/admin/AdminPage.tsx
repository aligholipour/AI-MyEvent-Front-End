...
function FilterChip({
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className={`px-3 py-1.5 rounded-xl text-[10.5px] font-black transition-all shrink-0 cursor-pointer flex items-center gap-1.5 ${
                active
...
function AdminPage({ onBack }: { onBack: () => void }) {
    ...
    const filteredUsers = users.filter((u) => {
        ...
        // if (userFilterVerified === 'verified') return u.isVerified;
        // if (userFilterVerified === 'unverified') return !u.isVerified;
        return true;
    });
    ...
    return (
        ...
        >
                ...
                <div className="flex bg-gray-100/80 p-1 rounded-2xl border border-gray-200/60">
                    ...
                    >
                        ...
                        <span className={`text-[9.5px] font-black px-1.5 py-0.2 rounded-md ${
                            pendingEventsCount > 0 ? 'bg-amber-500 text-white animate-pulse' : 'bg-gray-200 text-gray-700'
                        }`}>
                            {events.length}
                        </span>
                    </button>
                                                    ...
                                                    {event.status === 1 ? (
                                                        ...
                                                        <>
                                                            <button
                                                                ...
                                                                className={`px-3 py-1.5 rounded-xl text-[11px] font-black flex items-center gap-1.5 transition-all cursor-pointer border ${
                                                                    !event.isActive
            ...
            {/* ================= MODAL 3: USER DETAIL MODAL - AI Design ================= */}
            <AnimatePresence>
                {userDetail && (
                    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4">
                        <motion.div
